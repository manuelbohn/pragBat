...
#           iter = 4000)
# 
#           cor <- summary(model)$random$id%>%as_tibble(rownames = "parameter")%>%filter(parameter == "cor(test_dayday1,test_dayday2)")%>%mutate(task = i , study = "study1")
#           
...
#           iter = 4000)
# 
#           cor <- summary(model)$random$id%>%as_tibble(rownames = "parameter")%>%filter(parameter == "cor(test_dayday1,test_dayday2)")%>%mutate(task = i , study = "study2")
#           
#           model_cor <- bind_rows(model_cor, cor)
#   
# }
...
mcor <- readRDS("../saves/model_based_reliability.rds")%>%
  mutate(task = recode(task,
                    `Informativeness Inference (1)` = "Informativeness Inf. (1)",
                    `Informativeness Inference (2)` = "Informativeness Inf. (2)"),
         term = recode(term,
                    `Informativeness Inference (1)` = "Informativeness Inf. (1)",
                    `Informativeness Inference (2)` = "Informativeness Inf. (2)"))
  ...
  ad_model <- brm(correct ~  z_age + (0+test_day|id),
            data = data%>%filter(study == "study2", task == "ad_hoc_implicature"),
            family = bernoulli(),
          #control = list(adapt_delta = 0.99),
          sample_prior = F,
          chains = 4,
...
wide_data <- data %>%
  mutate(task = recode(task,
                    `mutual_exclusivity` = "Mutual Exclusivity",
                    `informativeness` = "Informativeness Inf. (1)",
                    `simple_inf` = "Informativeness Inf. (2)",
                    `preference` = "Speaker Preference",
                    `novelty` = "Discourse Novelty",
                    `card_sorting` = "Card Sorting",
                    `discourse_continuity` = "Discourse Continuity",
                    `ad_hoc_implicature` = "Ad-hoc Implicature",
                    `match_to_sample` = "Rel. Match-to-sample"))%>%
  filter(study != "study3",
         task != "training") %>%
  droplevels() %>%
...
cor_r1 <- data %>%
  ...
  mutate(study = "study1")%>%
  left_join(mcor)%>%
  mutate(cor2 = ifelse(!is.na(mcor), paste0(" ",cor,"\n",round(mcor,1)," (", round(mcor_lci,1),"-",round(mcor_uci,1), ")"), cor))
...
pf4 <- ggarrange(cp1, cp2, cp3, common.legend = F, nrow = 1, labels = c("A","B","C"), hjust = -1.5)

pf5 <- ggarrange(cp1, cp2, common.legend = F, ncol = 1, labels = c("A","B"))

ggsave(plot = pf5,"../paper/figures/figure4_1.png", width = 5, height = 10, scale = 1.1, bg = "white")
```

```{r}
ggsave(plot = cp3,"../paper/figures/figure6.png", width = 5, height = 5, scale = 1.1, bg = "white")
```
...
Here we run the three CFA models. We used slightly informative priors for the manifest and latent variable intercepts because Bayes Factors are conditional on the prior distributions used for the parameters. The results are very similar when default priors are being used. 

```{r}
# Priors
priors <- dpriors(nu = "normal(0,10)", 
                alpha = "normal(0,5)")

# run line below to get results with default priors
#priors <- dpriors()
```


```{r}
#load RDS objects to reproduce the results from the paper
fit <- bcfa(model, data = cfad, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 5000, dp = priors)
saveRDS(fit, "../saves/bcfa_prag.rds")
fit <- readRDS("../saves/bcfa_prag.rds")

fit2 <- bcfa(model2, data = cfad, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 5000, dp = priors)
saveRDS(fit2, "../saves/bcfa_single.rds")
fit2 <- readRDS("../saves/bcfa_single.rds")

fit3 <- bcfa(model3, data = cfad, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 5000, dp = priors)
saveRDS(fit3, "../saves/bcfa_separate.rds")
fit3 <- readRDS("../saves/bcfa_separate.rds")

base::summary(fit, ci = T, rsquare = T, standardized = T)
```

Here we extract the model estimates

```{r}
model_est <- coef(fit)%>%as_tibble(rownames = "index")%>%
  left_join(blavInspect(fit, "hpd", level = .95)%>%
              as_tibble(rownames = "index"))

write_csv(model_est, "../saves/cfa_estimates.csv")
...
Here we extract the WAIC value for each model and compute the WAIC weight.
...
# )%>%
#   arrange(waic)%>%
#   write_csv(., "../saves/bcfa_model_indices.csv")

model_indices <- read_csv("../saves/bcfa_model_indices.csv")

```

Here we compare models and extract the WAIC difference and standard error of the difference. We do the same for LOO.

```{r}
model_comparison <- bind_rows(
blavCompare(fit, fit2)$diff_waic%>%as_tibble(rownames = "param")%>%mutate(index = "waic", comparison = "prag vs. single"),
blavCompare(fit, fit2)$diff_loo%>%as_tibble(rownames = "param")%>%mutate(index = "loo", comparison = "prag vs. single"),
blavCompare(fit, fit2)$bf%>%as_tibble(rownames = "param")%>%mutate(index = "bf", comparison = "prag vs. single"),

blavCompare(fit, fit3)$diff_waic%>%as_tibble(rownames = "param")%>%mutate(index = "waic", comparison = "prag vs. separate"),
blavCompare(fit, fit3)$diff_loo%>%as_tibble(rownames = "param")%>%mutate(index = "loo", comparison = "prag vs. separate"),
blavCompare(fit, fit3)$bf%>%as_tibble(rownames = "param")%>%mutate(index = "bf", comparison = "prag vs. separate")
)

write_csv(model_comparison, "../saves/bcfa_model_comparison.csv")
...
For the CFA models reported above, we generate a continuous response variable for each task by z-transforming the proportion of correct responses for each task. An alternative perspective would be to treat the sum score as an ordinal variable. Below we report that models that treat the raw scores as ordered categories. 

```{r}
ocfad <- data%>%
  group_by(id,age, subage,task)%>%
  filter(task != "training")%>%
  summarise(mean = sum(correct))%>%
  pivot_wider(names_from = "task", values_from = "mean")%>%
  ungroup()%>%
  filter(!is.na(ad_hoc_implicature),
         !is.na(card_sorting),
...
We have to collapse several categories due to sparsity.
...
ocfad <- data%>%
  ...
  pivot_wider(names_from = "task", values_from = "sum")%>%
  ungroup()%>%
  select(-informativeness, -novelty,-preference, -discourse_continuity)%>%
  na.omit()%>%
  mutate(ad_hoc_implicature = recode(ad_hoc_implicature,
                                     `0` = 0,
                                     `1` = 0,
                                     `2` = 0,
                                     `3` = 1,
         ...
         card_sorting = recode(card_sorting,
                                     `0` = 0,
                                     `1` = 1,
                                     `2` = 1,
                                     `3` = 1,
        ...
        match_to_sample = recode(match_to_sample,
                                     `0` = 0,
                                     `1` = 0,
                                     `2` = 1,
                                     `3` = 1,
         ...
         mutual_exclusivity = recode(mutual_exclusivity,
                                     `0` = 0,
                                     `1` = 0,
                                     `2` = 0,
                                     `3` = 1,
...
We have to use very restrictive priors in order to get chains to converge

```{r}
# Priors
opriors <- dpriors(nu = "normal(0,2)",
                   alpha = "normal(0,2)",
                   lambda = "normal(0,2)",
                   beta = "normal(0,2)",
                tau = "normal(0, .5)")
...
```{r}
ofit <- bcfa(model, data = ocfad, ordered = T, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 10000, dp = opriors, mcmcextra = list(data = list(llnsamp = 50)))
# saveRDS(ofit, "../saves/obcfa_prag.rds")
# ofit <- readRDS("../saves/obcfa_prag.rds")

ofit2 <- bcfa(model2, data = ocfad, ordered = T, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 10000, dp = opriors, mcmcextra = list(data = list(llnsamp = 50)))
# saveRDS(ofit2, "../saves/obcfa_single.rds")
# ofit2 <- readRDS("../saves/obcfa_single.rds")

ofit3 <- bcfa(model3, data = ocfad, ordered = T, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 10000, dp = opriors, mcmcextra = list(data = list(llnsamp = 50)))
# saveRDS(ofit3, "../saves/obcfa_separate.rds")
# ofit3 <- readRDS("../saves/obcfa_separate.rds")
```


Thresholds for the pragmatic model

```{r}
coef(ofit)%>%as_tibble(rownames = "estimate")%>%
  filter(grepl("\\|", estimate))%>%
  separate(estimate, into = c("task", "thresh"), sep = "\\|")%>%
  ggplot(aes(x = task, y = value, col = thresh))+
  geom_point()+
  theme_minimal()
```
...
```{r}
bind_rows(
fitmeasures(ofit)%>%as_tibble(rownames = "index")%>%
  mutate(model = "ofit1"),
fitmeasures(ofit2)%>%as_tibble(rownames = "index")%>%
  mutate(model = "ofit2"),
fitmeasures(ofit3)%>%as_tibble(rownames = "index")%>%
 mutate(model = "ofit3")
)%>%
  pivot_wider(names_from = "model", values_from = "value")
...
ggsave(plot = prsa,"../paper/figures/figure7.png", width = 10, height = 4, scale = 1.25, bg = "white")
```
